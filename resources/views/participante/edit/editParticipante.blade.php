        ...
        <div class="card shadow-lg p-4 p-md-5 rounded-4" style="max-width: 800px; width: 90%; background: #f8f9fa; border: none; min-height: 400px;">
            <div class="text-center mb-4">
                <div class="avatar-container mb-3">
                    <img src="/images/avatar-placeholder.png" alt="Avatar" class="rounded-circle shadow" width="100">
                </div>
            ...
            <form action="{{ route('participante.update', $participante->id) }}" method="POST" class="needs-validation" novalidate>
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label class="form-label fw-semibold">Nome</label>
                    <input type="text" name="nome" class="form-control rounded-pill" value="{{ old('nome', $participante->nome) }}" required minlength="3">
                </div>

                <div class="mb-3">
                    <label class="form-label fw-semibold">CPF</label>
                    <input type="text" name="cpf" class="form-control rounded-pill" pattern="\d{3}\.\d{3}\.\d{3}-\d{2}" value="{{ old('cpf', $participante->cpf) }}" placeholder="000.000.000-00" required>
                </div>

                <div class="mb-3">
                    <label class="form-label fw-semibold">Data de Nascimento</label>
                    <input type="date" name="dataNasc" class="form-control rounded-pill" value="{{ old('dataNasc', $participante->dataNasc) }}" required>
                </div>

                <div class="mb-3">
                    <label class="form-label fw-semibold">E-mail</label>
                    <input type="email" name="email" class="form-control rounded-pill" value="{{ old('email', $participante->email) }}" required>
                </div>
            ...
            <div class="text-center mt-4 d-flex justify-content-between align-items-center">
                <a href="{{ route('participante.home') }}" class="text-decoration-none text-secondary fw-semibold">
